...
fun String?.convert2Long(defaultValue: Long, radix: Int = 10) =
    if (this.isNullOrBlank()) {
        ...
        this.lowercase(Locale.getDefault()).replace("l", "").toLong(radix)
    } catch (e: Exception) {
        defaultValue
    }
...
fun String?.convert2Float(defaultValue: Float) =
    if (this.isNullOrBlank()) {
        ...
        this.lowercase(Locale.getDefault()).replace("f", "").toFloat()
    } catch (e: Exception) {
        defaultValue
    }
...
fun String?.convert2Double(defaultValue: Double) =
    if (this.isNullOrBlank()) {
        ...
        this.lowercase(Locale.getDefault()).replace("f", "").toDouble()
    } catch (e: Exception) {
        defaultValue
    }
...
fun Double?.scientificNotation2str(): String? {
    val dStr = "$this"
    if (!dStr.lowercase(Locale.getDefault()).contains("e")) {
        return dStr
    }
